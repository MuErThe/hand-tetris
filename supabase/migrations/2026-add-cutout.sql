-- ============================================================
-- Migration: allow Cutout scores
-- ============================================================
-- Adds 'cutout' to the game whitelist in submit_game_score.
-- SAFE ON LIVE DATA: replaces one function body, touches no rows.
--
-- SUPERSEDES the double-take, contrast-call and steady-hand migrations —
-- this replaces the whole function and whitelists ALL FOUR new games, so
-- running only this one is enough. Running them in order is also fine.
--
-- Until this runs, the four new games' submissions are rejected with
-- 'unknown_game' and their boards stay empty. All four still play and
-- still record local progress.
--
