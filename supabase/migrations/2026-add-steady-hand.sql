-- ============================================================
-- Migration: allow Steady Hand scores
-- ============================================================
-- Adds 'steady-hand' to the game whitelist in submit_game_score.
-- SAFE ON LIVE DATA: replaces one function body, touches no rows.
--
-- SUPERSEDES the double-take and contrast-call migrations — this replaces
-- the whole function and whitelists ALL THREE new games, so running only
-- this one is enough. Running them in order is also fine.
--
-- Until this runs, Double Take, Contrast Call and Steady Hand submissions
-- are rejected with 'unknown_game' and their boards stay empty. All three
-- games still play and still record local progress.
--
...
declare
  ...
  v_max_plausible int;
begin
  if p_game not in ('tetris', 'eyeball-it', 'kern-combat', 'colour-forge', 'double-take', 'contrast-call', 'steady-hand') then
