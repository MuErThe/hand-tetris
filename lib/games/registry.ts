 ...
 *      `submit_game_score` function in supabase/schema.sql AND re-run that
 *      file in the Supabase SQL Editor: the server rejects unknown ids.
...
export interface GameDef {
  ...
  featured: boolean;
  /**
   * Gets a hub cell spanning the full grid width. The cell is far wider than
   * the vignette's own aspect, so its scene is capped to a centred window
   * rather than being scaled up to cover; see GameCard in app/page.tsx.
   */
  wide?: boolean;
...
export const GAMES: GameDef[] = [
  {
    ...
    meta: {
      ...
      description:
        "Gesture-controlled Tetris in the browser: steer with your hand, pinch to rotate, dip to drop. Webcam hand tracking runs entirely on-device, and video never leaves your browser.",
    },
    sitemapPriority: 0.7,
  },
  {
  ...
  },
  {
    ...
    trainsLong: "typographic craft in letter spacing",
    blurb:
  ...
  },
  {
    ...
    meta: {
      ...
      description:
        "Two versions of the same UI card, one with a single deliberate flaw: misalignment, uneven spacing, weak hierarchy, text under 4.5:1 contrast, a mismatched radius, lopsided padding, an off-centre icon, a mis-grouped label or an odd-sized sibling. Spot the clean one, then see the flaw named and measured.",
    },
    sitemapPriority: 0.9,
  },
  {
